...
class EncodingContainer_AnyCollectionTests: XCTestCase {
    func testEncodeDict() {
        let expectedSut = MockSerializableObject(dict: ["int": 123, "string": "string", "bool": true, "double": 873.436, "null": NSNull(), "array": [1, 2, 3, 4], "dict": ["string": "ok lah", "null": NSNull()]],
                                            array: [1, NSNull(), false, "string", 345.2346, ["1", "2", "4"], ["key": NSNull()]])
        var data: Data!
        var sut: MockSerializableObject!
        XCTAssertNoThrow(data = try JSONEncoder().encode(expectedSut))
        XCTAssertNoThrow(sut = try JSONDecoder().decode(MockSerializableObject.self, from: data))
    ...
    func testEncodeDictContainsDict() {
        let expectedSut = MockSerializableObject(dict: ["nestedDict": ["array":[123, NSNull(), true, 456.789, "string", [1, 2, 3], ["string": "string", "int": 456, "array": ["9", "8", "7"]]]]],
                                            array: [1, NSNull(), false, "string", 345.2346, ["1", "2", "4"], ["key": NSNull()]])
        var data: Data!
        var sut: MockSerializableObject!
        XCTAssertNoThrow(data = try JSONEncoder().encode(expectedSut))
    ...
    func testEncodeDictContainsArray() {
        let expectedSut = MockSerializableObject(dict: ["array": [123, NSNull(), true, 456.789, "string", [1, 2, 3], ["string": "string", "int": 456, "array": ["9", "8", "7"]]]],
                                            array: [1, NSNull(), false, "string", 345.2346, ["1", "2", "4"], ["key": NSNull()]])
        var data: Data!
        var sut: MockSerializableObject!
        XCTAssertNoThrow(data = try JSONEncoder().encode(expectedSut))
    ...
    func testEncodeIfPresentDict() {
        let expectedSut = MockSerializableIfPresentObject(dict: ["int": 123, "string": "cool", "bool": true, "double": 873.436, "null": NSNull(), "array": [1, 2, 3, 4], "dict": ["string": "ok lah", "null": NSNull()]],
                                                     array: nil)
        var data: Data!
        var sut: MockSerializableIfPresentObject!
        XCTAssertNoThrow(data = try JSONEncoder().encode(expectedSut))
        XCTAssertNoThrow(sut = try JSONDecoder().decode(MockSerializableIfPresentObject.self, from: data))
        XCTAssertEqual(sut.dict?["int"] as? Int, 123)
    ...
    func testEncodeIfPresentNil() {
        let expectedSut = MockSerializableIfPresentObject(dict: nil, array: nil)
        var data: Data!
        var sut: MockSerializableIfPresentObject!
        XCTAssertNoThrow(data = try JSONEncoder().encode(expectedSut))
        XCTAssertNoThrow(sut = try JSONDecoder().decode(MockSerializableIfPresentObject.self, from: data))
    ...
    func testDecodeIfPresentArray() {
        let expectedSut = MockSerializableIfPresentObject(dict: nil, array: [123, NSNull(), true, 456.789, "string", [1, 2, 3], ["string": "string", "int": 456, "array": ["9", "8", "7"]]])
        var data: Data!
        var sut: MockSerializableIfPresentObject!
        XCTAssertNoThrow(data = try JSONEncoder().encode(expectedSut))
        XCTAssertNoThrow(sut = try JSONDecoder().decode(MockSerializableIfPresentObject.self, from: data))
